...
switch(group_by)
    case 'type'
        ...
        subplot(1,3,1); hue_sig{1}=histogram(H_A,100); title('H (signal A)');
        subplot(1,3,2); Cb_sig{1}=histogram(Cb_A,100); title('Cb (signal A)');
        subplot(1,3,3); Cr_sig{1}=histogram(Cr_A,100); title('Cr (signal A)');
        figure % display histograms for signals of type B
        subplot(1,3,1); hue_sig{2}=histogram(H_B,100); title('H (signal B)');
        subplot(1,3,2); Cb_sig{2}=histogram(Cb_B,100); title('Cb (signal B)');
        subplot(1,3,3); Cr_sig{2}=histogram(Cr_B,100); title('Cr (signal B)');
        figure % display histograms for signals of type C
        subplot(1,3,1); hue_sig{3}=histogram(H_C,100); title('H (signal C)');
        subplot(1,3,2); Cb_sig{3}=histogram(Cb_C,100); title('Cb (signal C)');
        subplot(1,3,3); Cr_sig{3}=histogram(Cr_C,100); title('Cr (signal C)');
        figure % display histograms for signals of type D
        subplot(1,3,1); hue_sig{4}=histogram(H_D,100); title('H (signal D)');
        subplot(1,3,2); Cb_sig{4}=histogram(Cb_D,100); title('Cb (signal D)');
        subplot(1,3,3); Cr_sig{4}=histogram(Cr_D,100); title('Cr (signal D)');
        figure % display histograms for signals of type E
        subplot(1,3,1); hue_sig{5}=histogram(H_E,100); title('H (signal E)');
        subplot(1,3,2); Cb_sig{5}=histogram(Cb_E,100); title('Cb (signal E)');
        subplot(1,3,3); Cr_sig{5}=histogram(Cr_E,100); title('Cr (signal E)');
        figure % display histograms for signals of type F
        subplot(1,3,1); hue_sig{6}=histogram(H_F,100); title('H (signal F)');
        subplot(1,3,2); Cb_sig{6}=histogram(Cb_F,100); title('Cb (signal F)');
        subplot(1,3,3); Cr_sig{6}=histogram(Cr_F,100); title('Cr (signal F)');
...
save('Compute_Mask_Functions/Cb_sig','Cb_sig');
save('Compute_Mask_Functions/Cr_sig','Cr_sig');
