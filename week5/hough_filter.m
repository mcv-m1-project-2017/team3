...
% addpath('Compute_Mask_Functions\CircularHough_Grd');
dev_thr=5; %threshold in angle degrees

absolve=false;
candidate_edg=edge(candidate);
%     candidate_edg=edge(candidate,'canny',[0.05, 0.2],1);
...
obl_line = [];
 ...
 %triangle?
 if ((hor_num>=1)&&(vert_num>=2))
     absolve=true;
 end
 %circle?
 maximum_radius=min(length(candidate(:,1)),length(candidate(1,:)));
 minimum_radius=maximum_radius/2;
 radrange=[minimum_radius,maximum_radius];
%      [~, center] = CircularHough_Grd(double(candidate_edg), radrange);
 [center,rad] = imfindcircles(double(candidate_edg), radrange);
 if (~isempty(center))
    absolve=true; 
 end
