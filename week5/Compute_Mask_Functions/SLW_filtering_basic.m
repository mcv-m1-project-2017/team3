function [final_mask, WindowCandidates] = SLW_filtering_basic(original_mask, features)
...
for h = sides
    for a = aspect_ratios
        ...
        for i = min_i:1:max_i
            for j = min_j:1:max_j
                %sum of the pixels inside the current window if some
                %element is different from 0
                if (any(original_mask(i-ceil(h/2)+1:i+ceil(h/2)-1,j-ceil(w/2)+1:j+ceil(w/2)-1)))
                    fill_mat(i,j) = sum(sum(original_mask(i-ceil(h/2)+1:i+ceil(h/2)-1,j-ceil(w/2)+1:j+ceil(w/2)-1)));
                end
            end
        end
