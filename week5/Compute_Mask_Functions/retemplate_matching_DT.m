function [mask] = retemplate_matching_DT (mask, templates)
...
for i = 1:size(CC,1)
    ...
    [v,idx] = sort(v,'ascend');
    if ~(v(1) < 1000 && v(1) < 0.6*v(2))
        if (idx(1) == 5 && idx(2) == 6) || (idx(1) == 6 && idx(2) == 5)
        % do nothing, templates 5 and 6 are too similar
        else
            labels(labels==i) = 0;
            mask = labels>0;
        end
