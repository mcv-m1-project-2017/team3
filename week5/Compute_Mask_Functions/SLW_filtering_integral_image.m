function [final_mask, WindowCandidates] = SLW_filtering_integral_image(original_mask, features)
...
%compute integral image of the current mask
mask_ii = cumsum(cumsum(original_mask),2); 
mask_ii = padarray(mask_ii,[1 1],0,'pre'); 

fill_mat = zeros(size(original_mask));
for h = sides
    for a = aspect_ratios
        w = floor(a*h);
        if (mod(w,2) == 0), w = w + 1; end
        min_i = ceil(h/2); %in order to avoid the boundaries problem
        max_i = size(original_mask,1)-min_i + 1;
        min_j = ceil(w/2);
        max_j = size(original_mask,2)-min_j + 1;
        for i = (min_i+1):1:(max_i+1)
            for j = (min_j+1):1:(max_j+1)
                %computing sum within the window in the integral image
                fill_mat(i-1,j-1) =  mask_ii(i+ceil(h/2)-1,j+ceil(w/2)-1) ...
                                 + mask_ii(i-ceil(h/2),j-ceil(w/2)) ...
                                 - mask_ii(i-ceil(h/2),j+ceil(w/2)-1) ...
                                 - mask_ii(i+ceil(h/2)-1,j-ceil(w/2)); 
            end
        end
